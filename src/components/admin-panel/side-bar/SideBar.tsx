import { useLocation } from "react-router-dom";
import "./SideBar.css"

type SideBarProps = {
    selectPanel: (panel: "movies" | "venues") => void,
}

export default function SideBar({ selectPanel }: SideBarProps) {
    const location = useLocation();
    const activePanel = location.pathname.includes("venues") ? "venues" : "movies";
    
    return (
        <div className="side-bar-container">
            <h5 className="font-heading-h5" style={{ color: "#FCFCFD", marginBottom: "21px" }}>Admin</h5>
            <div className="full-width-horizontal-line" style={{ backgroundColor: "#667085", height: "1px" }}></div>
            <button
                className={`side-bar-btn ${activePanel === "movies" ? "font-lg-underline-semibold" : "font-lg-regular"}`}
                style={{ color: activePanel === "movies" ? "#FCFCFD" : "#D0D5DD" }}
                onClick={() => selectPanel("movies")}
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="#FCFCFD" width={16} height={24} viewBox="0 0 512 512"><path d="M0 96C0 60.7 28.7 32 64 32l384 0c35.3 0 64 28.7 64 64l0 320c0 35.3-28.7 64-64 64L64 480c-35.3 0-64-28.7-64-64L0 96zM48 368l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0c-8.8 0-16 7.2-16 16zm368-16c-8.8 0-16 7.2-16 16l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0zM48 240l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0c-8.8 0-16 7.2-16 16zm368-16c-8.8 0-16 7.2-16 16l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0zM48 112l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16L64 96c-8.8 0-16 7.2-16 16zM416 96c-8.8 0-16 7.2-16 16l0 32c0 8.8 7.2 16 16 16l32 0c8.8 0 16-7.2 16-16l0-32c0-8.8-7.2-16-16-16l-32 0zM160 128l0 64c0 17.7 14.3 32 32 32l128 0c17.7 0 32-14.3 32-32l0-64c0-17.7-14.3-32-32-32L192 96c-17.7 0-32 14.3-32 32zm32 160c-17.7 0-32 14.3-32 32l0 64c0 17.7 14.3 32 32 32l128 0c17.7 0 32-14.3 32-32l0-64c0-17.7-14.3-32-32-32l-128 0z" /></svg>
                Movies
            </button>
            <button
                className={`side-bar-btn ${activePanel === "venues" ? "font-lg-underline-semibold" : "font-lg-regular"}`}
                style={{ color: activePanel === "venues" ? "#FCFCFD" : "#D0D5DD" }}
                onClick={() => selectPanel("venues")}
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="#D0D5DD" width={12} height={24} viewBox="0 0 384 512"><path d="M64 48c-8.8 0-16 7.2-16 16l0 384c0 8.8 7.2 16 16 16l80 0 0-64c0-26.5 21.5-48 48-48s48 21.5 48 48l0 64 80 0c8.8 0 16-7.2 16-16l0-384c0-8.8-7.2-16-16-16L64 48zM0 64C0 28.7 28.7 0 64 0L320 0c35.3 0 64 28.7 64 64l0 384c0 35.3-28.7 64-64 64L64 512c-35.3 0-64-28.7-64-64L0 64zm88 40c0-8.8 7.2-16 16-16l48 0c8.8 0 16 7.2 16 16l0 48c0 8.8-7.2 16-16 16l-48 0c-8.8 0-16-7.2-16-16l0-48zM232 88l48 0c8.8 0 16 7.2 16 16l0 48c0 8.8-7.2 16-16 16l-48 0c-8.8 0-16-7.2-16-16l0-48c0-8.8 7.2-16 16-16zM88 232c0-8.8 7.2-16 16-16l48 0c8.8 0 16 7.2 16 16l0 48c0 8.8-7.2 16-16 16l-48 0c-8.8 0-16-7.2-16-16l0-48zm144-16l48 0c8.8 0 16 7.2 16 16l0 48c0 8.8-7.2 16-16 16l-48 0c-8.8 0-16-7.2-16-16l0-48c0-8.8 7.2-16 16-16z" /></svg>
                Venues
            </button>
        </div>
    )
}